...
// Calculate slot based on team index (1 match = 24 teams)
function calculateSlot(index: number): number {
    return Math.floor(index / 24) + 1
}
...
export async function GET() {
    try {
        ...
        const serializedForms = reversedForms.map((f, index) => {
            ...
            const matchNumber = Math.floor(index / 24) + 1
            const positionInMatch = (index % 24) + 1
        ...
        return NextResponse.json({
            ...
            totalMatches: Math.ceil(serializedForms.length / 24),
        })
    } catch (error) {
