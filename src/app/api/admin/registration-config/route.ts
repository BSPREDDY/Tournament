...
export async function GET() {
    try {
        ...
        const teamCountResult = await db.select().from(FormDataTable)
        const currentTeams = teamCountResult.length
        const currentMatches = Math.ceil(currentTeams / 25)
        ...
        if (configs.length === 0) {
            ...
            return Response.json({ ...newConfig[0], currentTeams, currentMatches }, { status: 201 })
        }

        return Response.json({ ...configs[0], currentTeams, currentMatches }, { status: 200 })
    } catch (error) {
        console.error("[v0] Get registration config error:", error)
        return Response.json({ error: "Failed to fetch registration config" }, { status: 500 })
    }
...
export async function PATCH(req: Request) {
    try {
        ...
        const { registrationStopAt, isRegistrationOpen, maxMatches } = body
        ...
        if (maxMatches !== undefined) {
            updateData.maxMatches = maxMatches
        }
        ...
        // Auto-close registration if max matches reached
        if (maxMatches && currentMatches >= parseInt(maxMatches)) {
            if (updatedConfig[0].isRegistrationOpen) {
                await db
                    .update(RegistrationConfigTable)
                    .set({ isRegistrationOpen: false })
        ...
        return Response.json({ ...updatedConfig[0], currentTeams, currentMatches }, { status: 200 })
    } catch (error) {
        console.error("[v0] Update registration config error:", error)
        return Response.json({ error: "Failed to update registration config" }, { status: 500 })
    }
