...
import { Label } from "@/src/components/ui/label"
import { toast } from "sonner"
import { Clock, Calendar, CheckCircle, XCircle, Save, Loader2, Trophy as TrophyIcon, Trophy, Users } from "lucide-react"

interface RegistrationConfig {
    id: string
    registrationStopAt: string | null
    isRegistrationOpen: boolean
    maxMatches: string | null
    currentTeams?: number
    ...
    const fetchMatches = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const totalMatches = Math.ceil(totalTeams / 25)

                const matchesData: Match[] = []
                for (let i = 1; i <= totalMatches; i++) {
                    const startIndex = (i - 1) * 25
                    const endIndex = Math.min(i * 25, totalTeams)
                    const teamCount = endIndex - startIndex
                    matchesData.push({ matchNumber: i, teamCount })
                }
                setMatches(matchesData)
            }
        } catch (error) {
            console.error("[v0] Failed to fetch matches:", error)
        }
    }
    ...
    const fetchRegistrationConfig = async () => {
        try {
            ...
            if (res.ok) {
                ...
                if (data.registrationStopAt) {
                    setRegistrationDeadline(new Date(data.registrationStopAt).toISOString().slice(0, 16))
                }

                if (data.maxMatches) {
                    setMaxMatches(data.maxMatches)
                }
            } else {
                console.error("[v0] API error:", res.status)
            }
    ...
    return (
        <div className="space-y-6 sm:space-y-8 max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
            ...
            <Card className="border-2 border-blue-500/20 shadow-lg">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-xl sm:text-2xl">
                        <Trophy className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500 flex-shrink-0" />
                        <span className="truncate">Matches Overview</span>
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    {matches.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                            {matches.map((match) => (
                                <div
                                    ...
                                    className="p-4 sm:p-5 rounded-lg bg-gradient-to-br from-blue-500/10 to-cyan-500/10 border border-blue-500/20 hover:border-blue-500/40 transition-all duration-300"
                                >
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-xs sm:text-sm text-muted-foreground mb-1">Match {match.matchNumber}</p>
                                            <p className="text-2xl sm:text-3xl font-bold text-blue-600 dark:text-blue-400">
                                                {match.teamCount}
                                            </p>
                                            <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                                                {match.teamCount === 25 ? "FULL" : `${25 - match.teamCount} slots left`}
                                            </p>
                                        </div>
                                        <Trophy className={`w-8 h-8 sm:w-10 sm:h-10 ${match.teamCount === 25 ? 'text-yellow-500' : 'text-blue-500/50'}`} />
                                    </div>
                                    <div className="mt-3 w-full bg-muted rounded-full h-2">
                                        <div
                                            className="bg-blue-500 h-2 rounded-full transition-all duration-300"
                                            style={{ width: `${(match.teamCount / 25) * 100}%` }}
                                        ></div>
                                    </div>
                                </div>
                            ))}
            ...
            {/* Set Max Matches Card */}
            <Card className="border-2 border-accent/20 shadow-lg">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-xl sm:text-2xl">
                        <Trophy className="w-5 h-5 sm:w-6 sm:h-6 text-accent flex-shrink-0" />
                        <span className="truncate">Maximum Matches Allowed</span>
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <form
                        onSubmit={async (e) => {
                            e.preventDefault()
                            if (!maxMatches || parseInt(maxMatches) <= 0) {
                                toast.error("Please enter a valid number of matches")
                                return
                            }
                            ...
                            try {
                                ...
                                if (res.ok) {
                                    const updatedConfig = await res.json()
                                    setConfig(updatedConfig)
                                    setMaxMatches(updatedConfig.maxMatches)
                                    toast.success(`Maximum matches updated to ${maxMatches}`)
                                } else {
                                    const errorData = await res.json()
                                    toast.error(errorData.error || "Failed to update max matches")
                                }
                            } catch (error) {
                                console.error("[v0] Update max matches error:", error)
                                toast.error("An error occurred while updating max matches")
                            } finally {
                                setIsSaving(false)
                            }
                        }}
                    ...
                    >
                        <div className="space-y-2 sm:space-y-3">
                            <Label htmlFor="maxMatches" className="text-sm sm:text-base font-semibold">
                                Number of Matches
                            </Label>
                            <p className="text-xs sm:text-sm text-muted-foreground">
                                Set the maximum number of matches (each match contains 25 teams). Registration will auto-close when this limit is reached.
                            </p>
                            <Input
                                id="maxMatches"
                                type="number"
                            ...
                            {maxMatches && (
                                <p className="text-xs text-muted-foreground mt-2">
                                    Tournament will accept up to <strong>{maxMatches}</strong> matches ({parseInt(maxMatches) * 25} teams)
                                </p>
                            )}
                        </div>
                        ...
                        <div className="space-y-3 sm:space-y-4">
                            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 sm:p-4">
                                ...
                                <p className="text-xs sm:text-sm text-muted-foreground">
                                    When the number of matches reaches the maximum allowed, registration will <strong>automatically close</strong>. Each match accommodates exactly 25 teams, so the calculation is automatic based on total registrations.
                                </p>
                            </div>
                            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 sm:p-4">
                                <p className="text-xs sm:text-sm text-muted-foreground">
                                    <strong>Note:</strong> You can still manually toggle registration status using the button above, regardless of whether the max matches limit is reached.
                                </p>
                            </div>
                        </div>
