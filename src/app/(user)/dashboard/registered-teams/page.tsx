            ...
            <div className="w-full pb-8 sm:pb-12">
                <div className="w-full max-w-7xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8">
                    <div className="space-y-8">
                        ...
                        {showSlotsAlert && (
                            <div className="p-4 sm:p-5 bg-gradient-to-r from-blue-500/15 to-cyan-500/15 border border-blue-500/40 rounded-xl flex items-start gap-3 group hover:border-blue-500/60 transition-all duration-300 shadow-lg shadow-blue-500/10 animate-[slideInDown_0.5s_ease-out]">
                                ...
                                <button
                                    ...
                                    onClick={() => setShowSlotsAlert(false)}
                                    className="flex-shrink-0 text-blue-600 hover:text-blue-700 dark:hover:text-blue-300 mt-0.5 transition-colors hover:bg-blue-500/20 p-1 rounded"
                                    aria-label="Close slot information"
                                    title="Close slot information"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        ...
                        {/* Teams by Matches View */}
                        <div className="space-y-6 sm:space-y-8">
                            {Array.from({ length: Math.ceil(teams.length / 25) }).map((_, matchIndex) => {
                                const matchNumber = matchIndex + 1
                                const startIdx = matchIndex * 25
                                const endIdx = Math.min((matchIndex + 1) * 25, teams.length)
                                const matchTeams = teams.slice(startIdx, endIdx)

                                return (
                                    <Card key={matchNumber} className="border-primary/20 overflow-hidden card-glow hover-lift animate-[slideInUp_0.5s_ease-out]">
                                        <CardHeader className="bg-gradient-to-r from-primary/10 via-secondary/5 to-transparent pb-3 sm:pb-4 border-b border-primary/10">
                                            <div className="flex items-center gap-2">
                                                <Trophy className="w-5 h-5 text-primary" />
                                                <CardTitle className="text-lg sm:text-xl font-bold">Match {matchNumber}</CardTitle>
                                                <span className="ml-auto px-3 py-1 bg-primary/20 text-primary rounded-full text-xs sm:text-sm font-semibold">
                                                    {matchTeams.length} / 25 Teams
                                                </span>
                                            </div>
                                        </CardHeader>
                                        <CardContent className="p-0">
                                                ...
                                                <table className="w-full text-xs sm:text-sm">
                                                    ...
                                                    <tbody className="divide-y divide-primary/5">
                                                        {matchTeams.map((team, index) => {
                                                            const overallIndex = startIdx + index
                                                            const maxSlots = 25

                                                            return <TeamTableRow key={team.id} team={team} index={overallIndex} maxSlots={maxSlots} />
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
