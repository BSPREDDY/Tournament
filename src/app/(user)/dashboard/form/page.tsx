...
//   return (
//     <div className="flex flex-col min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
//       <div className="sm:pt-24 md:pt-5 py-6 sm:py-8 md:py-12 flex-1 ">
...
export default function FormPage() {
  ...
  const [closureReason, setClosureReason] = useState<string | null>(null)
  const [user, setUser] = useState<User | null>(null)
  const [maxMatches, setMaxMatches] = useState<number | null>(null)
  const [currentTeams, setCurrentTeams] = useState<number>(0)
    ...
    const loadConfig = async () => {
      ...
      try {
        ...
        if (configRes.ok) {
          const config = await configRes.json()
          setCurrentTeams(config.currentTeams || 0)
          setCurrentMatches(config.currentMatches || 0)

          if (config.maxMatches) {
            const maxMatchesNum = parseInt(config.maxMatches, 10)
            setMaxMatches(maxMatchesNum)
            if (config.currentMatches >= maxMatchesNum) {
              setRegistrationClosed(true)
              setClosureReason("Maximum matches allowed has been reached")
              if (!toastShown) {
                toast.error("Maximum matches allowed has been reached. Registration is now closed.")
                toastShown = true
              }
              return
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Form submitted successfully!")
        // Clear form data and redirect after a short delay
        setTimeout(() => {
          router.push("/dashboard")
        }, 500)
      } else {
        ...
        if (data.details) {
          console.error("[v0] Validation errors:", data.details)
        }
      }
    } catch (error) {
          ...
          <div className="max-w-md mx-auto px-3 sm:px-4">
            <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-6 sm:p-8 text-center space-y-4">
              ...
              {maxMatches && currentMatches && (
                <div className="bg-white/50 dark:bg-black/20 rounded-lg p-3 my-4">
                  <p className="text-xs sm:text-sm font-semibold text-foreground">Matches Capacity</p>
                  <p className="text-lg sm:text-xl font-bold text-primary mt-1">{currentMatches} / {maxMatches}</p>
                </div>
              )}
              <p className="text-xs sm:text-sm text-muted-foreground">
                Please contact the administrator for more information.
