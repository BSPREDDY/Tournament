...
import { LogOut, LayoutDashboard, Users, FileText, Settings, Menu, X, Calendar, Mail, Lock, Video } from "lucide-react"
...
export function Sidebar() {
  ...
  const menuItems = [
    ...
    { href: "/admin/videos", label: "Videos Management", icon: Video },
  ...
  return (
    <>
      ...
      >
        <div className="mb-1 hidden lg:block flex-shrink-0 slide-in">
        ...
        <nav className="space-y-1.5 flex-1 overflow-y-auto min-h-0 pr-1 py-1 scrollbar-none">
              ...
              >
                <div
                  className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all duration-200 cursor-pointer ${
                    isActive
                      ? "bg-gradient-to-r from-primary to-secondary text-white font-bold shadow-lg shadow-primary/25"
                      : "text-white hover:text-white hover:bg-white/10 hover:translate-x-1"
                  }`}
                >
                  <Icon className="w-4 h-4 shrink-0 text-white" />
                  <span className="text-left text-xs sm:text-sm font-semibold text-white leading-tight">
                    {item.label}
                  </span>
                </div>
              </Link>
        ...
        <div className="space-y-2 flex-shrink-0 pt-3 border-t border-white/10">

          <button
            onClick={handleLogout}
            type="button"
            className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 transition-all duration-200 cursor-pointer"
          >
            <LogOut className="w-4 h-4 shrink-0 text-red-400" />
            <span className="text-left text-xs sm:text-sm font-semibold text-red-400">Logout</span>
          </button>
        </div>
      </aside>
    </>
